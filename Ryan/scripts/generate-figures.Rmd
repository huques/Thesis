...
estimateModels <- function(data, variables, dist.vars = "", collin = ""){
  ...
  constraints <- setdiff(constraints, collin)
...
# export table to html file with stargazer
...
```{R}
# remove all the transformations to variables for ease of formatting/identifying
pullVarname <- function(vars){
  sapply(vars, gsub, pattern = "\\(|\\)|I\\(|log|\\^2", replace = "") %>%
    unique()
}
...
car::vif(sfr_mods[[6]]) %>% as.data.frame() %>%
...
## QQ-plots of best SFR and MFR model
```{R}
# single
...
## Histogram of property values by property type
```{R}
...
## SFR non-linearity scatterplots
```{r}
...
## MFR building squarefootage imputation scatterplot
```{R}
