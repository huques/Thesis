...
Impsegcop has our attached garage, patio, basement, attic, and deck variables among others. Reshape to get the square footage of each segment type. For properties that have multiple decks, attics, etc, the square footage was summed by type, so currently there is no way to distinguish between properties that have multiple attics or just one really large one. We can think about whether proxying with a summed square footage is actually reasonable. 
```{r}
...
We need the columns PlumbingCode and Fire_place_Lookup from the impseg df. However, it appears that there are other variables such as perimeter, neighborhood marketvalue, total area, etc. that could be useful for our analysis. 
```{r}
glimpse(impseg)
...
dim(is_pruned) #pulled roughly 1% of the properties in impseg
dim(impseg)

is_pruned$PropertyID <- droplevels(is_pruned$PropertyID)

# there are 6901 multiple observations
multi_is <- is_pruned %>%
