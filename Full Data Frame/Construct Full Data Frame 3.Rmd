...
# join to dataframe
fugly <- sf::st_join(st_centroid(taxlots_pruned), walk, left = T)
...
# join to dataframe
fugly <- st_join(st_centroid(fugly), nbhd, left = TRUE) # change to a left join
...
# join to dataframe
fugly <- st_join(st_centroid(fugly), school, left = TRUE)
...
ftprints_pruned %<>% 
  ...
  dplyr::summarise(totalsqft = sum(BLDG_SQFT, na.rm = T),
            ...
            maxheight = max(AVG_HEIGHT, na.rm = T), #need extra cleaning
...
# join to dataframe
fugly <- st_join(st_centroid(fugly), sex, left = TRUE) %>%
...
df <- st_join(st_centroid(taxlots_pruned), zoningfeb2018, left = TRUE)

for(i in 1:(length(zonelist)-1)){
  df <- st_join(df, zonelist[[i]], left = TRUE)
}
