...
gdb3 <- here("DATA", "data_20191112.gdb")
...
gdb4 <- here("DATA", "tree_canopy.gdb")
...
gdb5 <- here("DATA", "canopy_20200210.gdb")

canopy <- st_read(gdb5, "canopy_taxlot_intersect")

#----------------------
# GDB 2/11 from PortlandMaps
walk_gdb <- here("DATA", "Complete_Neighborhoods_Scoring_Surface")
...
print(paste0("vacant Join: ", as.character(sum(data.frame(table(vacancy$STATE_ID))$Freq > 1))))
...
firstjoin <- st_join(st_centroid(taxlots_pruned), non_id_constraints, left = TRUE)
...
foot_flood <- st_read(here("DATA", "ft_fld.gdb"), layer = "ft_floodplain")

ftfld_ids <- foot_flood %>%
  filter(STATE_ID %in% stateids) %>%
  pull(STATE_ID)
...
# this new variable drops 20 properties from the original conFld100
```
...
```{r}
# Read
intersectionz <- st_read(here("DATA", "constraint_layers.gdb"),
                         "Intersectionz")
...
crosswalk <- read_xlsx(here("DATA", "zoning_crosswalk.xlsx"))
crosswalk %<>%
