---
title: "Single Family Residential Regression Results"
...
This chunk cleans the single-family residential data. It involves selecting the appropriate variables, and flagging certain variables in order to determine which transactions can be considered "arms length". This chunk also summarizes the multiple variables originally created for basements, attics, garages, etc. (the original dataset had multiple types of each) and creates a numeric and dummy version of each. 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
# Subset Data by Single Family Residential
SFR <- thesis_data %>%
...
This chunk looks at some of the control variables in order to determine any necessary changes to functional form, and to further inform cleaning. 
...
I have included two of my preliminary regressions here: a linear regression and a Box-Cox regression. Apologies that the results tables are so long! I had both neighborhood and high school districting fixed effects. 

```{r, echo = TRUE, mssage = FALSE}
