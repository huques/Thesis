...
library(sf)
library(tidyverse)
setwd("/Users/ryankobler/Desktop/thesis/Thesis/")
...
# using st_join instead: much faster!
...
zones_over_time %<>%
  mutate(
    correctZone = case_when(saledate >= as.Date("2014-02-01") & saledate < as.Date("2014-08-01") ~ ZONE_feb2014,
  ...
  mutate(correctZone = as.character(correctZone)) %>%
  mutate(correctZone = ifelse(is.na(correctZone), 
                              as.character(ZONE_aug2018), 
                              correctZone))

crosswalk$correctZone <- crosswalk$`Base Zone`

zones_over_time %<>%
  left_join(crosswalk, by = "correctZone")

zones_over_time %>%
  filter(is.na(correctZone) & saledate >= as.Date("2018-08-01")) %>%
  arrange(saledate) %>%
  select(saledate, ZONE_aug2018, STATE_ID)
